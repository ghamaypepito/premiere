...
const findTemplate = async (type) => {
  ...
  const html = await (await fetch('/wp-admin/edit.php?post_type=elementor_library&elementor_library_type=' + type,
    {credentials:'same-origin'})).text();
  const ids = [...html.matchAll(/id="post-(\\d+)"/g)].map(m => Number(m[1]));
